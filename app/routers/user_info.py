...
@router.get('/user_info/{username}/{google_sub}', response_model=UserInfo)
def login(username: str, google_sub: str, request: Request):
    user = user_info_resource.get_by_key(username)
    if not user or not google_sub == user.google_sub:
        raise HTTPException(status_code=401,
                            detail='Invalid username or Google Token')
...
def change_name(name_change: UsernameChangeRequest):
    ...
    if (not user or name_change.google_sub != user.google_sub or
...
def delete(user_info: UserInfo):
    ...
    if (not user or user_info.google_sub != user.google_sub):
        raise HTTPException(status_code=401,
                            detail='Invalid username or Google Token')
